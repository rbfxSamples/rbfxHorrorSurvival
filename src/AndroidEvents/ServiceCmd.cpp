...
extern "C"
{
    ...
    JNIEXPORT void JNICALL Java_org_libsdl_app_SDLActivity_SendServiceCommand(
        JNIEnv* env, jobject obj, jint cmd, jint status, jstring message)
    {
        if (handler)
        {
...
void ServiceCmd::SendCmdMessage(int cmd, int param) { Android_JNI_SendMessage(cmd, param); }
...
void ServiceCmd::SendResponseMsg(const MessageData& msg)
{
    ...
    eventData["Message"] =
        "Got cmd: " + ea::to_string(msg.command) + "; Status: " + ea::to_string(msg.status) + "; Msg: " + msg.message;
