...
void NakamaManager::LogIn(const ea::string& email, const ea::string& password)
{
    ...
    auto loginSucceededCallback = [&](NSessionPtr session)
    {
        ...
        data[P_TOKEN] = ea::string(session->getAuthToken().c_str());
        data[P_USER_ID] = ea::string(session->getUserId().c_str());
        data[P_USERNAME] = ea::string(session->getUsername().c_str());
...
{
    SendChatMessage("Hello, I'm " + ea::string(session_->getUsername().c_str()));
}
