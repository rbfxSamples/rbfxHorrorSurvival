...
void ModLoader::LoadASMods()
{
    ...
    GetSubsystem<FileSystem>()->ScanDir(result, GetSubsystem<FileSystem>()->GetProgramDir() + ea::string("Data/Mods"),
        ea::string("*.as"), SCAN_FILES, false);
    ...
    for (auto it = packageFiles.begin(); it != packageFiles.end(); ++it)
    {
        ...
        {
            if ((*it2).starts_with("Mods/") && (*it2).ends_with(".as") && (*it2).split('/').size() == 2)
            {
...
void ModLoader::LoadLuaMods()
{
    ...
    // Scan Data/Mods directory for all *.as files
    GetSubsystem<FileSystem>()->ScanDir(luaMods_, GetSubsystem<FileSystem>()->GetProgramDir() + ea::string("Data/Mods"),
        ea::string("*.lua"), SCAN_FILES, false);
    ...
    for (auto it = packageFiles.begin(); it != packageFiles.end(); ++it)
    {
        ...
        for (auto it2 = files.begin(); it2 != files.end(); ++it2)
        {
            if ((*it2).starts_with("Mods/") && (*it2).ends_with(".lua") && (*it2).split('/').size() == 2)
            {
...
void ModLoader::SubscribeConsoleCommands()
{
    ...
    SendEvent(E_CONSOLE_COMMAND_ADD, ConsoleCommandAdd::P_NAME, "mod_test", ConsoleCommandAdd::P_EVENT, "#mod_test",
        ConsoleCommandAdd::P_DESCRIPTION, "mod_test", ConsoleCommandAdd::P_OVERWRITE, true);
    SubscribeToEvent("#mod_test",
        [&](StringHash eventType, VariantMap& eventData)
        {
            ...
            if (params.size() >= 2)
            {
                ...
                if (file)
                {
                    ...
                    URHO3D_LOGINFOF("Reading file %s", content.c_str());
                }
                //            ea::string filename = params[1];
                //            SharedPtr<ScriptFile> file =
                //            SharedPtr<ScriptFile>(GetSubsystem<ResourceCache>()->GetResource<ScriptFile>(filename));
                //            if (!file) {
            ...
            //        URHO3D_LOGINFOF("Adding new resource filename=%s ; content=%s", filename.c_str(),
            //        content.c_str());
            ...
            //
            //        SharedPtr<ScriptFile> file =
            //        SharedPtr<ScriptFile>(GetSubsystem<ResourceCache>()->GetResource<ScriptFile>(filename.c_str()));
            //        URHO3D_LOGINFO(">>1");
            //        if (!file) {
...
void ModLoader::HandleReload(StringHash eventType, VariantMap& eventData) { Reload(); }
...
void ModLoader::HandleReloadScript(StringHash eventType, VariantMap& eventData)
{
    ...
    else
    {
        ...
        GetSubsystem<FileSystem>()->ScanDir(result,
            GetSubsystem<FileSystem>()->GetProgramDir() + ea::string("/Data/Mods"), ea::string("*.as"), SCAN_FILES,
            false);
