    ...
    , currentLevel_("NONE")
{
    ...
    SendEvent(E_CONSOLE_COMMAND_ADD, ConsoleCommandAdd::P_NAME, "change_level", ConsoleCommandAdd::P_EVENT,
        "ChangeLevelConsole", ConsoleCommandAdd::P_DESCRIPTION, "Change level");
    SubscribeToEvent("ChangeLevelConsole",
...
void LevelManager::HandleUpdate(StringHash eventType, VariantMap& eventData)
{
    ...
    if (fade_status_ == 3)
    {
        ...
        {
            URHO3D_LOGERROR(
                "Level '" + level_queue_.front() + "' doesn't exist in the system! Moving to 'MainMenu' level");
