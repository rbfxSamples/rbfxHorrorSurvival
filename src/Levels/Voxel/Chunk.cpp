...
void Chunk::Load()
{
    ...
    ea::string filename = "World/chunk_" + ea::string(position.x_) + "_" + ea::string(position.y_) + "_" + ea::string(position.z_) + ".json";
...
//    shouldRender_ = true;
//    renderIndex_ = 0;
////    URHO3D_LOGINFO("Chunk " + ea::string(position_) + " geometry calculated in " + ea::string(loadTime.GetMSec(false)) + "ms");
//}
...
//    URHO3D_LOGINFO("HandleHit blockposition: " + blockPosition.ToString() + "; neighbor position: " + neighborPosition.ToString());
    ...
    if (type != BT_AIR) {
        ...
        URHO3D_LOGINFO("Removing block " + blockPosition.ToString() + " Type: " + ea::string(static_cast<int>(type)) + "; Chunk position: " + position_.ToString());
...
//            + " Torch light: " + ea::string(GetTorchlight(blockPosition.x_, blockPosition.y_, blockPosition.z_)) +
//            "; Sun light: " + ea::string(GetSunlight(blockPosition.x_, blockPosition.y_, blockPosition.z_)));
...
void Chunk::Save()
{
    ...
    for (int x = 0; x < SIZE_X; ++x) {
        for (int y = 0; y < SIZE_Y; y++) {
            for (int z = 0; z < SIZE_Z; z++) {
                root.Set(ea::string(x) + "_" + ea::string(y) + "_" + ea::string(z), static_cast<int>(data_[x][y][z].type));
            }
        }
    }
    ...
    Vector3 position = Vector3(position_.x_ / SIZE_X, position_.y_ / SIZE_Y, position_.z_ / SIZE_Z);
    file.SaveFile("World/chunk_" + ea::string(position.x_) + "_" + ea::string(position.y_) + "_" + ea::string(position.z_) + ".json");
