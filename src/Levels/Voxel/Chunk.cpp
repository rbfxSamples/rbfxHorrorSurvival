...
void Chunk::RegisterObject(Context* context) { context->RegisterFactory<Chunk>(); }
...
void Chunk::Init(Scene* scene, const Vector3& position)
{
    ...
    SendEvent(E_CONSOLE_COMMAND_ADD, ConsoleCommandAdd::P_NAME, "chunk_rerender", ConsoleCommandAdd::P_EVENT,
        "#chunk_rerender", ConsoleCommandAdd::P_DESCRIPTION, "Rerender all chunks", ConsoleCommandAdd::P_OVERWRITE,
        true);
    SubscribeToEvent("#chunk_rerender",
...
void Chunk::Load()
{
    ...
    ea::string filename = "World/chunk_" + ea::string(position.x_) + "_" + ea::string(position.y_) + "_"
        + ea::string(position.z_) + ".json";
            ...
            for (int z = 0; z < SIZE_Z; z++)
            {
                ...
                for (int y = SIZE_Y - 1; y >= 0; y--)
                {
                    ...
                    if (surfaceHeight >= blockPosition.y_ && type == BT_DIRT)
                    {
                        ...
                        {
                            //                            GetSubsystem<TreeGenerator>()->AddTreeNode(x, y, z, 0, 0,
                            //                            this);
    ...
    //    URHO3D_LOGINFO("Chunk " + ea::string(position_) + " loaded in " + ea::string(loadTime.GetMSec(false)) + "ms");
    //    Save();
    loaded_ = true;
    shouldSave_ = true;
}
...
bool Chunk::Render()
{
    ...
    MutexLock lock(mutex_);
    {
        ...
        Material* material =
            SharedPtr<Material>(GetSubsystem<ResourceCache>()->GetResource<Material>("Materials/Voxel.xml"));
...

    {
        ...
        chunkObject->SetOccludee(true);
        Material* material =
            SharedPtr<Material>(GetSubsystem<ResourceCache>()->GetResource<Material>("Materials/VoxelWater.xml"));
        chunkObject->SetMaterial(material);
            ...
            for (int z = 0; z < SIZE_Z; z++)
            {
                ...
                if (!shouldDelete_)
                {
                    ...
                    if (!BlockHaveNeighbor(BlockSide::TOP, x, y, z))
                    {
                        ...
                        mesh->AddVertex(MeshVertex{position + Vector3(0.0f, 1.0, 0.0f), Vector3::UP, color,
                            GetTextureCoord(BlockSide::TOP, static_cast<BlockType>(type), Vector2(0.0, 0.0))});
                        mesh->AddVertex(MeshVertex{position + Vector3(0.0f, 1.0, 1.0), Vector3::UP, color,
                            GetTextureCoord(BlockSide::TOP, static_cast<BlockType>(type), Vector2(0.0, 1.0))});
                        mesh->AddVertex(MeshVertex{position + Vector3(1.0, 1.0, 0.0f), Vector3::UP, color,
                            GetTextureCoord(BlockSide::TOP, static_cast<BlockType>(type), Vector2(1.0, 0.0))});
                        mesh->AddVertex(MeshVertex{position + Vector3(1.0, 1.0, 1.0f), Vector3::UP, color,
                    ...
                    if (!BlockHaveNeighbor(BlockSide::BOTTOM, x, y, z))
                    {
                        ...
                        color.r_ = static_cast<int>(light & 0xF) / 15.0f;
                        color.g_ = static_cast<int>((light >> 4) & 0xF) / 15.0f;
                        mesh->AddVertex(MeshVertex{position + Vector3(0.0f, 0.0f, 1.0), Vector3::DOWN, color,
                            GetTextureCoord(BlockSide::BOTTOM, static_cast<BlockType>(type), Vector2(0.0, 1.0))});
                        mesh->AddVertex(MeshVertex{position + Vector3(0.0f, 0.0f, 0.0f), Vector3::DOWN, color,
                            GetTextureCoord(BlockSide::BOTTOM, static_cast<BlockType>(type), Vector2(0.0, 0.0))});
                        mesh->AddVertex(MeshVertex{position + Vector3(1.0, 0.0f, 0.0f), Vector3::DOWN, color,
                            GetTextureCoord(BlockSide::BOTTOM, static_cast<BlockType>(type), Vector2(1.0, 0.0))});
                        mesh->AddVertex(MeshVertex{position + Vector3(1.0, 0.0f, 1.0), Vector3::DOWN, color,
                    ...
                    if (!BlockHaveNeighbor(BlockSide::LEFT, x, y, z))
                    {
                        ...
                        color.r_ = static_cast<int>(light & 0xF) / 15.0f;
                        color.g_ = static_cast<int>((light >> 4) & 0xF) / 15.0f;
                        mesh->AddVertex(MeshVertex{position + Vector3(0.0f, 0.0f, 1.0), Vector3::LEFT, color,
                            GetTextureCoord(BlockSide::LEFT, static_cast<BlockType>(type), Vector2(0.0, 1.0))});
                        mesh->AddVertex(MeshVertex{position + Vector3(0.0f, 1.0, 1.0), Vector3::LEFT, color,
                            GetTextureCoord(BlockSide::LEFT, static_cast<BlockType>(type), Vector2(0.0, 0.0))});
                        mesh->AddVertex(MeshVertex{position + Vector3(0.0f, 0.0f, 0.0f), Vector3::LEFT, color,
                            GetTextureCoord(BlockSide::LEFT, static_cast<BlockType>(type), Vector2(1.0, 1.0))});
                        mesh->AddVertex(MeshVertex{position + Vector3(0.0f, 1.0, 0.0f), Vector3::LEFT, color,
                    ...
                    if (!BlockHaveNeighbor(BlockSide::RIGHT, x, y, z))
                    {
                        ...
                        color.r_ = static_cast<int>(light & 0xF) / 15.0f;
                        color.g_ = static_cast<int>((light >> 4) & 0xF) / 15.0f;
                        mesh->AddVertex(MeshVertex{position + Vector3(1.0, 0.0f, 0.0f), Vector3::LEFT, color,
                            GetTextureCoord(BlockSide::RIGHT, static_cast<BlockType>(type), Vector2(0.0, 1.0))});
                        mesh->AddVertex(MeshVertex{position + Vector3(1.0, 1.0, 0.0f), Vector3::LEFT, color,
                            GetTextureCoord(BlockSide::RIGHT, static_cast<BlockType>(type), Vector2(0.0, 0.0))});
                        mesh->AddVertex(MeshVertex{position + Vector3(1.0, 0.0f, 1.0), Vector3::LEFT, color,
                            GetTextureCoord(BlockSide::RIGHT, static_cast<BlockType>(type), Vector2(1.0, 1.0))});
                        mesh->AddVertex(MeshVertex{position + Vector3(1.0, 1.0, 1.0), Vector3::LEFT, color,
...
//                             geometry->DefineTexCoord(GetTextureCoord(BlockSide::TOP, static_cast<BlockType>(blockId),
//                             Vector2(0.0, 0.0))); geometry->DefineTangent(Vector4(0.0f, 0.0f, -1.0f, -1.0f));
//                         }
//
//                         {
//                             unsigned char light = NeighborLightValue(BlockSide::TOP, x, y, z);
...
//                             geometry->DefineNormal(Vector3::UP);
//                             geometry->DefineTexCoord(GetTextureCoord(BlockSide::TOP, static_cast<BlockType>(blockId),
//                             Vector2(0.0, 1.0))); geometry->DefineTangent(Vector4(0.0f, 0.0f, -1.0f, -1.0f));
//                         }
//
//                         {
//                             unsigned char light = NeighborLightValue(BlockSide::TOP, x, y, z);
...
//                             geometry->DefineNormal(Vector3::UP);
//                             geometry->DefineTexCoord(GetTextureCoord(BlockSide::TOP, static_cast<BlockType>(blockId),
//                             Vector2(1.0, 0.0))); geometry->DefineTangent(Vector4(0.0f, 0.0f, -1.0f, -1.0f));
//                         }
//
//                         // tri2
//                         {
...
//                             geometry->DefineNormal(Vector3::UP);
//                             geometry->DefineTexCoord(GetTextureCoord(BlockSide::TOP, static_cast<BlockType>(blockId),
//                             Vector2(1.0, 1.0))); geometry->DefineTangent(Vector4(0.0f, 0.0f, -1.0f, -1.0f));
//                         }
//
//                         {
//                             unsigned char light = NeighborLightValue(BlockSide::TOP, x, y, z);
...
//                             geometry->DefineTexCoord(GetTextureCoord(BlockSide::BOTTOM,
//                             static_cast<BlockType>(blockId), Vector2(0.0, 1.0)));
...
//                             geometry->DefineNormal(Vector3::DOWN);
//                             geometry->DefineTexCoord(GetTextureCoord(BlockSide::BOTTOM,
//                             static_cast<BlockType>(blockId), Vector2(0.0, 0.0)));
//                             geometry->DefineTangent(Vector4(1.0f, 0.0f, 0.0f, 1.0f));
//                         }
//
//                         {
...
//                             geometry->DefineNormal(Vector3::DOWN);
//                             geometry->DefineTexCoord(GetTextureCoord(BlockSide::BOTTOM,
//                             static_cast<BlockType>(blockId), Vector2(1.0, 0.0)));
//                             geometry->DefineTangent(Vector4(1.0f, 0.0f, 0.0f, 1.0f));
//                         }
//
//                         // tri2
...
//                             geometry->DefineNormal(Vector3::DOWN);
//                             geometry->DefineTexCoord(GetTextureCoord(BlockSide::BOTTOM,
//                             static_cast<BlockType>(blockId), Vector2(1.0, 1.0)));
//                             geometry->DefineTangent(Vector4(1.0f, 0.0f, 0.0f, 1.0f));
//                         }
//
//                         {
...
//                             geometry->DefineTexCoord(GetTextureCoord(BlockSide::LEFT,
//                             static_cast<BlockType>(blockId), Vector2(0.0, 1.0)));
//                             geometry->DefineTangent(Vector4(1.0f, 0.0f,  0.0f,  1.0f));
//                         }
//
...
//                            geometry->DefineColor(color);
//                            geometry->DefineNormal(Vector3::LEFT);
//                            geometry->DefineTexCoord(GetTextureCoord(BlockSide::LEFT, static_cast<BlockType>(blockId),
//                            Vector2(0.0, 0.0))); geometry->DefineTangent(Vector4(1.0f, 0.0f,  0.0f,  1.0f));
//                        }
//
//                        {
//                            unsigned char light = NeighborLightValue(BlockSide::LEFT, x, y, z);
...
//                            geometry->DefineNormal(Vector3::LEFT);
//                            geometry->DefineTexCoord(GetTextureCoord(BlockSide::LEFT, static_cast<BlockType>(blockId),
//                            Vector2(1.0, 0.0))); geometry->DefineTangent(Vector4(1.0f, 0.0f,  0.0f,  1.0f));
//                        }
//
//                        {
//                            unsigned char light = NeighborLightValue(BlockSide::LEFT, x, y, z);
...
//                            geometry->DefineNormal(Vector3::LEFT);
//                            geometry->DefineTexCoord(GetTextureCoord(BlockSide::LEFT, static_cast<BlockType>(blockId),
//                            Vector2(1.0, 1.0))); geometry->DefineTangent(Vector4(1.0f, 0.0f,  0.0f,  1.0f));
//                        }
//                    }
//                    if (!BlockHaveNeighbor(BlockSide::RIGHT, x, y, z)) {
//                        // tri1
...
//                            geometry->DefineTexCoord(GetTextureCoord(BlockSide::RIGHT,
//                            static_cast<BlockType>(blockId), Vector2(0.0, 1.0)));
...
//                            geometry->DefineTexCoord(GetTextureCoord(BlockSide::FRONT,
//                            static_cast<BlockType>(blockId), Vector2(0.0, 1.0)));
//                            geometry->DefineTangent(Vector4(1.0f, 0.0f,  0.0f,  1.0f));
//                        }
//
...
//                            geometry->DefineTexCoord(GetTextureCoord(BlockSide::BACK, static_cast<BlockType>(blockId),
//                            Vector2(0.0, 1.0))); geometry->DefineTangent(Vector4(1.0f, 0.0f,  0.0f,  1.0f));
//                        }
//
//                        {
//                            unsigned char light = NeighborLightValue(BlockSide::BACK, x, y, z);
...
//    shouldRender_ = true;
//    renderIndex_ = 0;
////    URHO3D_LOGINFO("Chunk " + ea::string(position_) + " geometry calculated in " +
///ea::string(loadTime.GetMSec(false)) + "ms");
//}
...
void Chunk::HandleHit(StringHash eventType, VariantMap& eventData)
{
    ...
    //    URHO3D_LOGINFO("HandleHit blockposition: " + blockPosition.ToString() + "; neighbor position: " +
    ...
    if (type != BT_AIR)
    {
        ...
        URHO3D_LOGINFO("Removing block " + blockPosition.ToString() + " Type: " + ea::string(static_cast<int>(type))
            + "; Chunk position: " + position_.ToString());
...
void Chunk::SetBlockData(const IntVector3& blockPosition, BlockType type)
{
    ...
    {
        SetTorchlight(blockPosition.x_, blockPosition.y_, blockPosition.z_, 0);
        GetSubsystem<LightManager>()->AddLightRemovalNode(
            blockPosition.x_, blockPosition.y_, blockPosition.z_, lightLevel, this);
    }
    for (int i = 0; i < 6; i++)
    {
...
Vector3 Chunk::NeighborBlockWorldPosition(BlockSide side, IntVector3 blockPosition)
{
    ...
    switch (side)
    {
    case TOP: return position + Vector3::UP;
    case BOTTOM: return position + Vector3::DOWN;
    case LEFT: return position + Vector3::LEFT;
    case RIGHT: return position + Vector3::RIGHT;
    case FRONT: return position + Vector3::BACK;
    case BACK: return position + Vector3::FORWARD;
    }
    return position;
}
...
void Chunk::HandleAdd(StringHash eventType, VariantMap& eventData)
{
    ...
    if (data_[blockPosition.x_][blockPosition.y_][blockPosition.z_].type == BlockType::BT_AIR)
    {
        ...
        {
            URHO3D_LOGINFOF("Render count=%d, geometry calculated=%d, should render=%d", renderCount_,
                IsGeometryCalculated(), ShouldRender());
            //            URHO3D_LOGINFO("Chunk selected: " + position_.ToString() + "; block: " +
            //            blockPosition.ToString()
            //            + " Torch light: " + ea::string(GetTorchlight(blockPosition.x_, blockPosition.y_,
            //            blockPosition.z_)) +
            //            "; Sun light: " + ea::string(GetSunlight(blockPosition.x_, blockPosition.y_,
            //            blockPosition.z_)));

            //            Color color;
            //            // Torchlight
            //            color.r_ = static_cast<int>(GetSunlight(blockPosition.x_, blockPosition.y_, blockPosition.z_)
            //            & 0xF) / 15.0f;
            //            // Sunlight
            //            color.g_ = static_cast<int>((GetSunlight(blockPosition.x_, blockPosition.y_, blockPosition.z_)
            //            >> 4) & 0xF) / 15.0f; URHO3D_LOGINFO("Color : " + color.ToString());
            return;
        }
...
const Vector3& Chunk::GetPosition() { return position_; }
...
void Chunk::Save()
{
    ...
    for (int x = 0; x < SIZE_X; ++x)
    {
        for (int y = 0; y < SIZE_Y; y++)
        {
            ...
            {
                root.Set(
                    ea::string(x) + "_" + ea::string(y) + "_" + ea::string(z), static_cast<int>(data_[x][y][z].type));
            }
        }
    }
    ...
    Vector3 position = Vector3(position_.x_ / SIZE_X, position_.y_ / SIZE_Y, position_.z_ / SIZE_Z);
    file.SaveFile("World/chunk_" + ea::string(position.x_) + "_" + ea::string(position.y_) + "_"
        + ea::string(position.z_) + ".json");
...
void Chunk::MarkForDeletion(bool value) { shouldDelete_ = value; }

bool Chunk::IsMarkedForDeletion() { return shouldDelete_; }

void Chunk::MarkActive(bool value) { isActive_ = value; }

bool Chunk::IsActive() { return isActive_; }
...
BlockType Chunk::GetBlockValue(int x, int y, int z) { return data_[x][y][z].type; }
...
int Chunk::GetSunlight(int x, int y, int z) { return (lightMap_[x][y][z] >> 4) & 0xF; }
...
int Chunk::GetTorchlight(int x, int y, int z) { return lightMap_[x][y][z] & 0xF; }
...
unsigned char Chunk::GetLightValue(int x, int y, int z) { return lightMap_[x][y][z]; }
...
void Chunk::MarkForGeometryCalculation() { calculateIndex_++; }

int Chunk::GetPartIndex(int x, int y, int z) { return Floor(x / (SIZE_X / (PART_COUNT - 1))); }
...
void Chunk::SetDistance(int distance) { distance_ = distance; }

const int Chunk::GetDistance() const { return distance_; }
...
bool Chunk::IsRequestedFromServer() { return requestedFromServer_; }
...
bool Chunk::ShouldRender() { return shouldRender_; }

bool Chunk::IsLoaded() { return loaded_; }

bool Chunk::IsGeometryCalculated() { return calculateIndex_ == lastCalculatateIndex_; }

bool Chunk::ShouldSave() { return shouldSave_; }
#endif
