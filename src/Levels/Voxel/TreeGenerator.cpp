...
void TreeGenerator::RegisterObject(Context* context) { context->RegisterFactory<TreeGenerator>(); }
...
void TreeGenerator::Process()
{
    ...
    while (!treeBfsQueue_.empty())
    {
        ...
        for (int i = 0; i < 6; i++)
        {
            ...
            int dWidth = width;
            if (side == BlockSide::FRONT || side == BlockSide::BACK || side == BlockSide::LEFT
