...
void ChunkMesh::RegisterObject(Context* context) { context->RegisterFactory<ChunkMesh>(); }
...
void ChunkMesh::AddVertex(const MeshVertex& vertexData) { vertices_.push_back(vertexData); }

void ChunkMesh::AddIndice(short index) { indices_.push_back(index); }
...
unsigned ChunkMesh::GetVertexCount() { return vertices_.size(); }
