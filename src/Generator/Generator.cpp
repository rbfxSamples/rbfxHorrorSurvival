...
{
    SendEvent(E_CONSOLE_COMMAND_ADD, ConsoleCommandAdd::P_NAME, "generate_map", ConsoleCommandAdd::P_EVENT,
        "#generate_map", ConsoleCommandAdd::P_DESCRIPTION, "Perlin noise map generating",
        ConsoleCommandAdd::P_OVERWRITE, true);
    SubscribeToEvent("#generate_map",
    ...
    SendEvent(E_CONSOLE_COMMAND_ADD, ConsoleCommandAdd::P_NAME, "save_heightmap", ConsoleCommandAdd::P_EVENT,
        "#save_heightmap", ConsoleCommandAdd::P_DESCRIPTION, "Save generated image to file");
    SubscribeToEvent("#save_heightmap", [&](StringHash eventType, VariantMap& eventData) { Save(); });

    // Register our loading step
    SendEvent(SceneManagerEvents::E_REGISTER_LOADING_STEP, SceneManagerEvents::RegisterLoadingStep::P_NAME,
        "Generating world", SceneManagerEvents::RegisterLoadingStep::P_EVENT, "GenerateWorld");

    // Handle our loading step
    SubscribeToEvent("GenerateWorld",
        [&](StringHash eventType, VariantMap& eventData)
        {
            SendEvent(SceneManagerEvents::E_ACK_LOADING_STEP, SceneManagerEvents::RegisterLoadingStep::P_EVENT,
                "GenerateWorld");
            GenerateImage(40, 1, Random());
            Save();
            SendEvent(SceneManagerEvents::E_LOADING_STEP_FINISHED, SceneManagerEvents::RegisterLoadingStep::P_EVENT,
                "GenerateWorld");
        });

    SendEvent(E_CONSOLE_COMMAND_ADD, ConsoleCommandAdd::P_NAME, "generate_ui_textures", ConsoleCommandAdd::P_EVENT,
        "#generate_ui_textures", ConsoleCommandAdd::P_DESCRIPTION, "Generate UI textures");
    SubscribeToEvent("#generate_ui_textures",
        [&](StringHash eventType, VariantMap& eventData)
        {
...
void Generator::Save() { generatedImage_->SavePNG("Data/Textures/HeightMap.png"); }
