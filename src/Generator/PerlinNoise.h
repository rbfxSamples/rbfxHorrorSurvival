...
public:
    explicit PerlinNoise(int seed) { reseed(seed); }
    ...
    double noise(double x, double y, double z) const
    {
        ...
        return Lerp(w,
            Lerp(v, Lerp(u, Grad(randomNumbers[AA], x, y, z), Grad(randomNumbers[BA], x - 1, y, z)),
                Lerp(u, Grad(randomNumbers[AB], x, y - 1, z), Grad(randomNumbers[BB], x - 1, y - 1, z))),
            Lerp(v, Lerp(u, Grad(randomNumbers[AA + 1], x, y, z - 1), Grad(randomNumbers[BA + 1], x - 1, y, z - 1)),
    ...
    double octaveNoise0_1(double x, int octaves) const { return octaveNoise(x, octaves) * 0.5 + 0.5; }

    double octaveNoise0_1(double x, double y, int octaves) const { return octaveNoise(x, y, octaves) * 0.5 + 0.5; }
