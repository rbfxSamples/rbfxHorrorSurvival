...
void UIOption::RegisterObject(Context* context)
{
    ...
    URHO3D_MIXED_ACCESSOR_ATTRIBUTE("Option Name", GetOptionName, SetOptionName, ea::string, "", AM_DEFAULT);
}

UIOption::UIOption(Context* context)
    : BorderImage(context)
{
    SetFixedHeight(40);
...
void UIOption::SetOptionName(const ea::string& name)
{
    option_name_ = name;
    if (label_)
...
void UIBoolOption::SetOptionLabel(bool state, const ea::string& text)
{
...
ea::string UIBoolOption::GetOptionLabel(bool state) const
{
    auto label = GetChildDynamicCast<Text>(state ? "BtnOff" : "BtnOn", true);
    if (label)
...
void UIMultiOption::RegisterObject(Context* context)
{
    ...
    URHO3D_MIXED_ACCESSOR_ATTRIBUTE(
        "Option Strings", GetOptionsAttr, SetOptionsAttr, VariantVector, Variant::emptyVariantVector, AM_DEFAULT)
...
void UIMultiOption::SetNumberOfOptions(int count) { options_count_ = count; }
...
UISliderOption::UISliderOption(Context* context)
    : UIOption(context)
    , range_(1.f)
    , value_(0.f)
{
...
void UITabPanel::RegisterObject(Context* context)
{
    ...
    URHO3D_MIXED_ACCESSOR_ATTRIBUTE(
        "Tabs", GetTabsAttribute, SetTabsAttribute, VariantVector, Variant::emptyVariantVector, AM_DEFAULT)
        .SetMetadata(AttributeMetadata::P_VECTOR_STRUCT_ELEMENTS, tabVectorNames);
}

UITabPanel::UITabPanel(Context* context)
    : BorderImage(context)
    , tab_count_(0)
    , current_tab_(0)
{
...
void UITabPanel::RemoveTab(int index)
{
    ...
    // Verify tabs_.erase(index, 1);
    tabs_.erase_first(tab);
