...
void BaseLevel::SubscribeToBaseEvents()
{
    ...
    SendEvent(E_CONSOLE_COMMAND_ADD, ConsoleCommandAdd::P_NAME, "gamma", ConsoleCommandAdd::P_EVENT, "gamma",
        ConsoleCommandAdd::P_DESCRIPTION, "Change gamma", ConsoleCommandAdd::P_OVERWRITE, true);
    SubscribeToEvent("gamma",
    ...
    SendEvent(E_CONSOLE_COMMAND_ADD, ConsoleCommandAdd::P_NAME, "clip", ConsoleCommandAdd::P_EVENT, "clip",
        ConsoleCommandAdd::P_DESCRIPTION, "Change camera far/near clip", ConsoleCommandAdd::P_OVERWRITE, true);
    SubscribeToEvent("clip",
        [&](StringHash eventType, VariantMap& eventData)
        {
            StringVector params = eventData["Parameters"].GetStringVector();
            if (params.size() == 3)
            {
                float nearClip = ToFloat(params[1]);
                float farClip = ToFloat(params[2]);
                for (int i = 0; i < cameras_.size(); i++)
                {
                    cameras_[i]->GetComponent<Camera>()->SetNearClip(nearClip);
                    cameras_[i]->GetComponent<Camera>()->SetFarClip(farClip);
                    URHO3D_LOGINFOF("Updating camera %d, near=%f, far=%f", i, nearClip, farClip);
                }
            }
        });

    SubscribeToEvent("postprocess", [&](StringHash eventType, VariantMap& eventData) { ApplyPostProcessEffects(); });

    SubscribeToEvent(
        E_SCREENMODE, [&](StringHash eventType, VariantMap& eventData) { SendEvent(E_VIDEO_SETTINGS_CHANGED); });
    ...
    if (scene_)
    {
        ...
        if (data_.contains("Commands"))
        {
            ...
            for (auto it = commands.begin(); it != commands.end(); ++it)
            {
                ...
                SendEvent(E_CONSOLECOMMAND, P_ID, "0", P_COMMAND, (*it));
            }
        }
    }
}
...
void BaseLevel::SubscribeToEvents()
{
    ...
    SendEvent(E_CONSOLE_COMMAND_ADD, ConsoleCommandAdd::P_NAME, "ambient_light", ConsoleCommandAdd::P_EVENT,
        "#ambient_light", ConsoleCommandAdd::P_DESCRIPTION, "Change scene ambient light",
        ConsoleCommandAdd::P_OVERWRITE, true);
    SubscribeToEvent("#ambient_light",
        [&](StringHash eventType, VariantMap& eventData)
        {
            if (!scene_)
            {
    ...
    SendEvent(E_CONSOLE_COMMAND_ADD, ConsoleCommandAdd::P_NAME, "fog", ConsoleCommandAdd::P_EVENT, "#fog",
        ConsoleCommandAdd::P_DESCRIPTION, "Change custom scene fog", ConsoleCommandAdd::P_OVERWRITE, true);
    SubscribeToEvent("#fog",
        [&](StringHash eventType, VariantMap& eventData)
        {
            if (!scene_)
            {
...
ea::vector<IntRect> BaseLevel::InitRects(int count)
{
    ...
    else if (count == 4)
    {
        ...
        // Bottom right
        rects.push_back(
            IntRect(graphics->GetWidth() / 2, graphics->GetHeight() / 2, graphics->GetWidth(), graphics->GetHeight()));
    }
    else if (count == 5)
    {
...
void BaseLevel::ApplyPostProcessEffects()
{
    ...
    for (int i = 0; i < GetSubsystem<Renderer>()->GetNumViewports(); i++)
    {
        ...
        effectRenderPath->SetEnabled(
            "AutoExposure", GetSubsystem<ConfigManager>()->GetBool("postprocess", "AutoExposure", false));
        effectRenderPath->SetShaderParameter("AutoExposureAdaptRate",
            GetSubsystem<ConfigManager>()->GetFloat("postprocess", "AutoExposureAdaptRate", 0.1f));
        effectRenderPath->SetEnabled("Bloom", GetSubsystem<ConfigManager>()->GetBool("postprocess", "Bloom", false));
        effectRenderPath->SetEnabled(
            "BloomHDR", GetSubsystem<ConfigManager>()->GetBool("postprocess", "BloomHDR", false));
...
#if !defined(__EMSCRIPTEN__)
        effectRenderPath->SetEnabled(
            "GammaCorrection", GetSubsystem<ConfigManager>()->GetBool("postprocess", "GammaCorrection", true));
#endif

        effectRenderPath->SetEnabled(
            "ColorCorrection", GetSubsystem<ConfigManager>()->GetBool("postprocess", "ColorCorrection", false));
        float gamma = Clamp(GAMMA_MAX_VALUE - GetSubsystem<ConfigManager>()->GetFloat("postprocess", "Gamma", 1.0f),
            0.05f, GAMMA_MAX_VALUE);
